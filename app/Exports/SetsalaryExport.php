...
class SetsalaryExport implements FromCollection, WithHeadings, WithMapping
{
    ...
    public function collection()
    {
        // $salarys = Employee::with('designation')->get();
        $salarys = CompanyWiseSalary::with('branch', 'employee', 'company_client', 'roles')->groupBy('employee_id')->get();

        return $salarys;
    }

    public function map($salarys): array
    {
        // return [
        //     $salarys->id,
        //     $salarys->name,
        ...
        // ];
        return [
            $salarys->employee->name ?? "-",
            $salarys->branch->name ?? "-",
            $salarys->company->name ?? "-",
            $salarys->roles->name ?? "-",
            $salarys->salary,
            $salarys->hra,
            $salarys->washing_allowances
        ];
    }
